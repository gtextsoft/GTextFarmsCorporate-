...
async function createFieldOfficer() {
  const email = (process.env.FIELD_EMAIL ?? "field@gtextfarms.ng").toLowerCase();
  const password = process.env.FIELD_PASSWORD ?? "ChangeMeField123!";
  const fullName = process.env.FIELD_NAME ?? "GText Farms Field Officer";
